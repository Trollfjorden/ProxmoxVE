...
if [[ ! "${prompt,,}" =~ ^(n|no)$ ]]; then
  ...
  pnpm_desired=$(grep -Po '"pnpm":\s*"\K[^"]+' /opt/riven-frontend/package.json)
  NODE_VERSION="24" NODE_MODULE="pnpm@$pnpm_desired" setup_nodejs

  msg_info "Installing Riven Frontend"
  $STD git -C /opt/riven-frontend rev-parse HEAD > /opt/riven_frontend_version.txt
