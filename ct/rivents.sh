...
function update_script() {
  ...
  if [[ "$RELEASE" != "$current" ]]; then
    ...
    cp /opt/rivents/apps/riven/.env.riven /tmp/.env.riven 2>/dev/null || true
    cp /opt/rivents/packages/plugin-seerr/.env.seerr /tmp/.env.seerr 2>/dev/null || true
    cp /opt/rivents/packages/plugin-plex/.env.plex /tmp/.env.plex 2>/dev/null || true
    msg_ok "Backed up Data"

    rm -rf /opt/rivents/{apps,packages,node_modules}
    mkdir -p /opt/rivents
    # Sync folders instead of full copy to avoid omitting hidden files without glob config
    shopt -s dotglob
    ...
    cp /tmp/.env.riven /opt/rivents/apps/riven/.env.riven 2>/dev/null || true
    cp /tmp/.env.seerr /opt/rivents/packages/plugin-seerr/.env.seerr 2>/dev/null || true
    cp /tmp/.env.plex /opt/rivents/packages/plugin-plex/.env.plex 2>/dev/null || true
...
echo -e "${INFO}${YW} Access it using its IP!${CL}"
